#!/usr/bin/env python3
"""
KODA Confirmation LIVE Bot — Bybit V5 API + Confirmation Stage
Merges: paper_bot_confirm.py (signal engine) + live_bot.py (Bybit execution)

Usage:
    python3 live_bot_confirm.py              # Dry-run mode (logs what WOULD happen)
    python3 live_bot_confirm.py --live       # LIVE mode (places real orders on Bybit!)

Strategy:
    - Full 7-score analysis (delta, OB, funding, distance, walls, POC, VA)
    - Score gate: >= 3/7 aligned
    - BTC Cascade Ampel: >= 2 lights required
    - BTC MTF Gate: 1H SMA20 vs SMA50
    - BTC Spike Filter: >1% in 15min blocks new trades
    - Confirmation Stage: signal must move 0.3% in predicted direction within 8 min
    - TP1/TP2 trailing: 50% close at TP1, SL to BE, 2% trail from peak
    - Adaptive SL: EMA ribbon width tightens SL when trend weakens
    ...
    - Dual mode: --live for Bybit, default dry-run with local JSON tracking

Config: $15/trade, 10x leverage, $1000 budget, 30% SL (price move)
"""
...
CONFIG = {
    "capital": 15,            # $15 per trade
    "leverage": 10,           # 10x
    "min_probability": 60,
    "tp_range_pct": 70,       # 70% of expected move (TP1)
    "sl_price_pct": 30,       # 30% PRICE move
    "total_budget": 1000,
    "tf_budget_15m": 50,
    "tf_budget_30m": 50,
    "tf_budget_1h": 0,
...
_CFG_OVERRIDE = os.path.join(BASE_DIR, "live_bot_confirm_config.json")
...
COINS = [
    # Tier 1: Proven in paper trading
    "GLM", "CYS", "SYN", "AXL", "TON", "DUSK",
    "IOST", "CELR", "GRT", "KAS",
    # Tier 2: Top screener picks
    "ID", "OP", "ENJ", "MOVR", "MORPHO", "BERA",
    "JUP", "KMNO", "AXS", "ORCA",
]

TZ = timezone(timedelta(hours=-4))
DATA_FILE = os.path.join(BASE_DIR, "live_trades.json")
LOG_FILE = os.path.join(BASE_DIR, "live_bot_confirm.log")
STATUS_FILE_PATH = os.path.join(BASE_DIR, "live_bot_status.json")
...
# ═══════════════════════════════════════════════════════════════
CONFIRM_PCT = 0.003     # 0.3% move in predicted direction required
CONFIRM_BARS = 8        # within 8 bars (8 minutes — checked every minute)
_pending_signals = []   # Stage 1 queue: waiting for confirmation
...
# ═══════════════════════════════════════════════════════════════
# COINS — Bybit symbol mapping + quantity precision
# ═══════════════════════════════════════════════════════════════
...
TG_CHANNEL_ID = "-1003770314055"  # Signal channel (confirmed signals only)
...
def _load_signal_counter():
    ...
    try:
        with open("/tmp/koda_live_confirm_signal_counter.txt", "r") as f:
...
def _save_signal_counter():
    try:
        with open("/tmp/koda_live_confirm_signal_counter.txt", "w") as f:
...
def send_tg_channel(text):
    """Send to signal channel (confirmed signals only)."""
    send_tg(TG_CHANNEL_ID, text)
...
def api(url, timeout=10):
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "LiveBotConfirm/1.0"})
...
def full_analyze(coin, tf="15m", limit=800):
    ...
    # Score gate: need >= 3/7 (ms3 from confirmation optimization)
    ...
    if btc_trend == "BULLISH":
        if coin_bias == "LONG":
            direction = "LONG"; probability = prob_a
        elif coin_bias == "SHORT":
            direction = "SHORT"; probability = prob_c
        else:
            direction = "LONG"; probability = prob_a
    ...
    if os.path.exists(DATA_FILE):
        try:
            ...
            data.setdefault("config", {}).update({
                ...
                "sl_price_pct": CONFIG.get("sl_price_pct", 30),
                "bot_name": "KODA Confirm LIVE",
...
def calc_sl_price(entry, direction):
    """Calculate SL price based on sl_price_pct PRICE move."""
    sl_price_pct = CONFIG.get("sl_price_pct", 30) / 100.0
    if direction == "LONG":
...
def send_drawdown_alert(sl_count):
    ...
    text = (f"DRAWDOWN-BREMSE AKTIV -- KODA Confirm LIVE Bot\n\n"
...
def notify_trade_closed(trade, data=None):
    ...
    if pnl > 0:
        header = f"TRADE CLOSED -- WIN [{mode}]"
        result_line = f"+${pnl:.2f} ({roi:+.1f}%)"
    else:
        header = f"TRADE CLOSED -- LOSS [{mode}]"
        result_line = f"${pnl:.2f} ({roi:+.1f}%)"
...
{result_line}
{'='*30}{footer}
KODA Confirm LIVE | {datetime.now(TZ).strftime('%H:%M')} ET"""
...
def send_hourly_status(data):
    ...
    msg = f"""KODA Confirm [{mode}] -- Stundenbericht
{'='*30}
...
def open_trade(data, tf_key, coin, direction, entry, tp, probability, tf, cascade_lights=0, cascade_code="00000"):
    ...
    log(f"  OPENED {direction} {coin} @ {entry:.6f} | TP: {tp:.6f} | SL: {sl:.6f} (30% price) | Liq: {liq:.6f} | Prob: {probability}% | TF: {tf} | Cascade: {cascade_lights}")
...
def close_trade(trade, close_price, reason):
    """Close a trade. Tracks consecutive SLs for drawdown brake."""
    ...
    if pnl <= 0:
        ...
        if _consecutive_sl_count >= DRAWDOWN_BRAKE_SL_COUNT and not _drawdown_paused:
            ...
            log(f"DRAWDOWN BRAKE ACTIVATED -- {_consecutive_sl_count} losses in a row! No new trades.")
...
def check_adaptive_sl(trade, klines_for_coin):
    ...
    if direction == "LONG":
        ...
        current_sl = trade.get("sl", entry * (1 - 0.30))
...
def check_collective_profit_exit(data):
    """
    Collective Profit Exit:
    If ANY single trade has ROI > 30% AND sum of all profitable ROIs >= 100%:
    Close all profitable trades (that have tp1_hit), starting from highest ROI.
    """
    all_open = []
    for tfk in ["trades_15m", "trades_30m"]:
        for t in data.get(tfk, []):
            if t["status"] == "open" and t.get("tp1_hit", False):
                all_open.append(t)
    ...
    # Check conditions
    max_roi = max(t.get("roi", 0) or 0 for t in profitable)
    sum_roi = sum(t.get("roi", 0) or 0 for t in profitable)

    if max_roi > 30 and sum_roi >= 100:
        log(f"  COLLECTIVE PROFIT EXIT: Max ROI {max_roi:.1f}%, Sum ROI {sum_roi:.1f}% -- closing all profitable trades")

        # Sort by ROI descending
        profitable.sort(key=lambda t: t.get("roi", 0) or 0, reverse=True)
        ...
        for trade in profitable:
            ...
            # Calculate final PnL including tp1_pnl
            if trade.get("tp1_pnl"):
                tp2_pnl = calc_pnl(trade["direction"], trade["entry"], current_price, trade["size"]) * 0.5
                total_pnl = trade["tp1_pnl"] + tp2_pnl
            else:
                total_pnl = calc_pnl(trade["direction"], trade["entry"], current_price, trade["size"])

            trade["pnl"] = round(total_pnl, 2)
            trade["roi"] = round(total_pnl / trade["margin"] * 100, 2)
            trade["close_price"] = round(current_price, 8)
            trade["close_time"] = datetime.now(TZ).strftime("%Y-%m-%dT%H:%M:%S")
            trade["close_reason"] = "COLLECTIVE_PROFIT"
            trade["status"] = "closed"

            log(f"  COLLECTIVE CLOSED: {trade['direction']} {coin} | PnL: ${total_pnl:.2f} ({trade['roi']:.1f}%)")
            notify_trade_closed(trade, _current_data)

            time.sleep(0.1)

        # Telegram summary
        total_pnl_all = sum(t.get("pnl", 0) or 0 for t in profitable if t["status"] == "closed")
        mode = "LIVE" if LIVE_MODE else "DRY-RUN"
        send_tg_chris(f"COLLECTIVE PROFIT EXIT [{mode}]\n{len([t for t in profitable if t['status']=='closed'])} Trades geschlossen\nGesamt: ${total_pnl_all:+.2f}")


# ═══════════════════════════════════════════════════════════════
# CHECK OPEN TRADES (TP1/TP2/SL/LIQ/24h Timeout)
# ═══════════════════════════════════════════════════════════════

def check_open_trades(data):
    """
    Check all open trades for TP1/TP2 trailing, SL, liquidation, 24h timeout.
    ...
    for tf_key in ["trades_15m", "trades_30m"]:  # Only 15m and 30m active
        ...
        for trade in open_trades:
            ...
            if coin in klines_cache and not trade.get("tp1_hit", False):
                ...
                if adaptive_sl is not None:
                    ...
                    if not trade.get("sl_tightened"):
                        ...
                        # Update SL on Bybit if live
                        if LIVE_MODE and trade.get("mode") == "LIVE":
                            set_tp_sl(bybit_sym, sl_price=round_price(sl_price))
            ...
            # ── TP1/TP2 Trailing Stop Logic (2% trail, NOT 3%) ──
            ...
            if trade["direction"] == "LONG":
                if not tp1_hit:
                    ...
                    if recent_high >= trade["tp"]:
                        # TP1 hit — close 50%, move SL to BE, start trailing
                        ...
                        if LIVE_MODE and trade.get("mode") == "LIVE":
                            ...
                            time.sleep(0.3)
                            # Move SL to BE (entry + fees)
                            be_price = round_price(trade["entry"] * 1.0015)
                        ...
                        log(f"  SL HIT: {coin} LONG | Low {recent_low:.6f} <= SL {sl_price:.6f}")
                    ...
                    if recent_low <= be_stop:
                        ...
                        trade["status"] = "closed"
                        log(f"  TP2 BE: {coin} LONG | Back to BE | TP1: ${trade.get('tp1_pnl', 0):.2f} + TP2: $0.00 = ${total_pnl:.2f}")
                        notify_trade_closed(trade, _current_data)
                ...
                if not tp1_hit:
                    if recent_low <= trade["tp"]:
                        ...
                        if LIVE_MODE and trade.get("mode") == "LIVE":
                            ...
                            time.sleep(0.3)
                            be_price = round_price(trade["entry"] * 0.9985)
                            set_tp_sl(bybit_sym, sl_price=be_price)
                            log(f"  [LIVE] SL moved to BE: {coin} @ {be_price}")
                        ...
                        close_trade(trade, sl_price, "SL")
                        log(f"  SL HIT: {coin} SHORT | High {recent_high:.6f} >= SL {sl_price:.6f}")
                    ...
                    # Trailing stop: 2% retrace from peak (for SHORT, price going UP is bad)
                    trail_stop = peak * (1 + 0.02)
                    # BE stop: entry - fees
                    ...
                    if recent_high >= be_stop:
                        ...
                        trade["close_reason"] = "TP1+BE"
                        trade["status"] = "closed"
                        log(f"  TP2 BE: {coin} SHORT | Back to BE | TP1: ${trade.get('tp1_pnl', 0):.2f} + TP2: $0.00 = ${total_pnl:.2f}")
                        notify_trade_closed(trade, _current_data)
    ...
    # Check collective profit exit after all individual checks
...
def update_stats(data):
    ...
    for tf_key, stats_key in [("trades_15m", "stats_15m"), ("trades_30m", "stats_30m"),
        ...
        wins = [t for t in closed if t.get("close_reason") in ("TP", "TP1+TRAIL", "TP1+BE", "COLLECTIVE_PROFIT")]
...
def scan_and_trade(data, tf, limit, tf_key):
    """Scan all coins and queue signals for confirmation."""
    now = datetime.now(TZ)
    ...
    # Drawdown brake
    if _drawdown_paused:
        log(f"  DRAWDOWN BRAKE ACTIVE -- {_consecutive_sl_count} SLs in Folge. Keine neuen Trades.")
        log(f"  Offene Trades werden weiter ueberwacht. Manueller Restart noetig.")
        return
    ...
    # Also exclude coins with pending confirmation
    ...
    if budget_available < CONFIG["capital"]:
        log(f"  Budget: ${current_budget:.0f} | Margin used: ${margin_used:.0f} | Frei: ${budget_available:.0f} -- kein Platz.")
        return
    ...
    for coin in COINS:
        ...
        try:
            ...
            if probability >= CONFIG["min_probability"]:
                ...
                log(f"          Cascade: {bull_lights}B/{bear_lights}S -> {cascade_dir} | Lights: {lights_in_dir} | {cascade_details}")
                ...
                # CONFIRMATION STAGE: Don't open immediately -- add to pending queue
    ...
    log(f"\nScan complete: {signals_found} signals found, {trades_queued} queued for confirmation")
...
def write_bot_status():
    ...
    try:
        ...
        status = {
            ...
            "api_ok": api_ok,
            "config": {k: CONFIG[k] for k in ("capital", "leverage", "min_probability", "tp_range_pct", "sl_price_pct")},
...
def write_positions_file():
    """Write Bybit positions to live_positions.json."""
    if not LIVE_MODE:
        return
    try:
        positions = get_positions()
        ...
        pos_file = os.path.join(BASE_DIR, 'live_positions.json')
...
def main():
    mode_label = "LIVE" if LIVE_MODE else "DRY-RUN"
    log(f"{'='*60}")
    log(f"  KODA Confirmation LIVE Bot starting in {mode_label} mode")
    log(f"{'='*60}")
    ...
    log(f"SL: {CONFIG['sl_price_pct']}% PRICE | TP: {CONFIG['tp_range_pct']}% Expected Move | MTF Gate: BTC 1H")
    log(f"CONFIRMATION: {CONFIRM_PCT*100:.1f}% in {CONFIRM_BARS} bars required before entry")
    log(f"TP1/TP2: 50% close at TP1, SL->BE(+fees), 2% trail from peak")
    log(f"24h Force Close: No TP1 after 24h -> close at market")
    log(f"Collective Profit Exit: ROI>30% single + sum>=100% -> close all profitable")
    log(f"Drawdown Brake: {DRAWDOWN_BRAKE_SL_COUNT} SLs in Folge -> Pause + Telegram")
    log(f"Score gate: >= 3/7 | Budget: ${CONFIG['total_budget']}")
    ...
    # Write PID file
    try:
        with open("/tmp/live_bot_confirm.pid", "w") as f:
    ...
    while True:
        try:
            ...
            if minute % 5 == 0:
                try:
                    bot_dir = BASE_DIR
                    subprocess.run(["git", "add", "live_trades.json", "live_positions.json", "live_bot_status.json"],
                                   cwd=bot_dir, capture_output=True, timeout=10)
                    subprocess.run(["git", "commit", "-m", f"KODA Confirm LIVE data update ({mode_label})"],
                                   cwd=bot_dir, capture_output=True, timeout=10)
            ...
            try:
                ...
                os.remove("/tmp/live_bot_confirm.pid")
            except Exception:
                pass
            send_tg_chris(f"KODA Confirm Bot gestoppt [{mode_label}]")
