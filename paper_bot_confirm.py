#!/usr/bin/env python3
"""
KODA Confirmation Bot — Full Stack + Observer/Executor
Config: 5x|70%TP|ms3 + Confirmation(0.3%/8bars)
Best tested config: $14,747 PnL (+77% vs baseline), 0 SL on 1m verification
Signal → 0.3% Bestätigung in 8 Bars → Entry → TP1/TP2/Trailing
"""
...
_CFG_OVERRIDE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "paper_bot_confirm_config.json")
...
DATA_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "paper_trades_confirm.json")

# ═══════════════════════════════════════════════════════════════
# CONFIRMATION STAGE CONFIG
# ═══════════════════════════════════════════════════════════════
CONFIRM_PCT = 0.003     # 0.3% move in predicted direction required
CONFIRM_BARS = 8        # within 8 bars (2h on 15m TF)
...
def log(msg):
    ...
    print(f"[{ts}] [CONFIRM] {msg}", flush=True)
...
def check_pending_confirmations(data):
    ...
    for sig in _pending_signals:
        ...
        if confirmed:
            # Entry at current price (confirmed level)
...
def main():
    log("KODA Confirmation Bot — Observer/Executor starting...")
    log(f"Strategy: 5x Leverage, $50/trade, $1000 Budget")
    log(f"SL: 42% PRICE | TP: 70% Expected Move | MTF Gate: BTC 1H")
    ...
    log(f"Score gate: >= 3/7 | Min probability: {CONFIG['min_probability']}%")
