...
DATA_FILE = os.path.join(SCRIPT_DIR, "paper_trades_optimal.json")
SIGNALS_FILE = os.path.join(SCRIPT_DIR, "ee2_signals.json")
LOG_FILE = os.path.join(SCRIPT_DIR, "..", "ee2_trader_b.log")
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [EE2-TEST-B] %(message)s",
    handlers=[
...
def git_push():
    """Push paper_trades_optimal.json every 5 min."""
    global _last_push
    now = time.time()
    ...
    try:
        ...
        subprocess.run(["git", "add", "paper_trades_optimal.json"],
