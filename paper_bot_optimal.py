#!/usr/bin/env python3
"""
Paper Trading Bot KODA Optimal — Best optimized strategy from V6
$1k Budget, $50/Trade, 2x Leverage, 42% Price SL, 55% Expected Move TP
MTF Gate: BTC 1H SMA20 vs SMA50 alignment required
Start: 07.06.2026
"""
...
_CFG_OVERRIDE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "paper_bot_optimal_config.json")
...
DATA_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "paper_trades_optimal.json")
...
def log(msg):
    ...
    print(f"[{ts}] [OPTIMAL] {msg}", flush=True)
...
def main():
    log("Paper Trading Bot KODA Optimal starting...")
    log(f"Strategy: 2x Leverage, $50/trade, $1000 Budget")
    log(f"SL: 42% PRICE move (= 84% margin loss at 2x)")
    log(f"TP: 55% Expected Move | MTF Gate: BTC 1H SMA20 vs SMA50")
    log(f"Score gate: >= 4/7 | Min probability: {CONFIG['min_probability']}%")
