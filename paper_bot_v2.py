...
CONFIG = {
    ...
    "tf_budget_15m": 40,      # % of total budget for 15m trades
    "tf_budget_30m": 60,      # % of total budget for 30m trades
...
_CFG_OVERRIDE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "paper_bot_v2_config.json")
...
DATA_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "paper_trades_v2.json")
    ...
    while True:
        try:
            ...
            if minute % 5 == 0:
                try:
                    import subprocess
                    subprocess.run(["git", "add", "paper_trades_v2.json"], cwd=os.path.dirname(DATA_FILE), capture_output=True, timeout=10)
