#!/usr/bin/env python3
"""
Paper Trading Bot V3 — SL 40%, TP 60%, Min Prob 65%
Same logic as paper_bot.py but with tighter parameters.
"""
...
CONFIG = {
    "capital": 100,
    "leverage": 10,
    "min_probability": 65,    # V3: Min Prob 65%
    "tp_range_pct": 60,       # V3: TP bei 60% of expected move
    "sl_pct": 40,             # V3: SL bei 40% Verlust der Margin
    "max_open_15m": 50,
...
def main():
    log("Paper Trading Bot V3 starting...")
    ...
    while True:
        try:
            ...
            if minute % 5 == 0:
                try:
                    import subprocess
                    subprocess.run(["git", "add", "paper_trades_v3.json"], cwd=os.path.dirname(DATA_FILE), capture_output=True, timeout=10)
                    subprocess.run(["git", "commit", "-m", "Paper bot V3 data update"], cwd=os.path.dirname(DATA_FILE), capture_output=True, timeout=10)
